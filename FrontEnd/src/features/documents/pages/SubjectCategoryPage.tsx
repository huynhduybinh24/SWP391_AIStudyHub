    ...
    <div className="space-y-8 pb-12 animate-fade-in">
      ...
      <div className="space-y-4 pt-2">
        <button 
          ...
          className="flex items-center gap-1.5 text-sm font-semibold text-slate-400 hover:text-[#2563eb] dark:hover:text-blue-400 transition-colors focus:outline-none w-fit"
        >
        ...
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-center">
          <div className="flex items-center gap-4">
            {/* Terminal folder box */}
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#EBF1FF] dark:bg-blue-955/40 text-[#2563eb] dark:text-blue-400 border border-blue-100/50 dark:border-blue-900/30 shadow-xs animate-pulse-slow">
              <svg className="h-7 w-7 text-[#2563eb] dark:text-blue-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 mt-1 flex-wrap text-slate-400 dark:text-slate-500">
                <span className="inline-flex items-center rounded-full bg-[#EBF1FF] dark:bg-blue-955/40 px-2.5 py-0.5 text-[10px] font-bold text-[#2563eb] dark:text-blue-400 border border-blue-100/50 dark:border-blue-900/30 uppercase tracking-wider">
                  {activeSubjectId === 'COMPSCI' ? '12' : subjectDocuments.length} DOCUMENTS
                </span>
                <span className="text-slate-300 dark:text-slate-800">•</span>
                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Active Course: {subjectInfo.courseCode}</span>
              </div>
            </div>
          </div>
          ...
          <div 
            ...
            className={cn(
              ...
              activeSubjectId === 'COMPSCI'
                ? "border-indigo-100 bg-[#EEF2FF]/60 hover:border-indigo-300 dark:border-indigo-900/50 dark:bg-indigo-950/20 dark:hover:border-indigo-800"
                : "border-blue-100 bg-blue-50/20 hover:border-blue-300 dark:border-blue-900/50 dark:bg-blue-955/20 dark:hover:border-blue-800"
            )}
          >
            {/* Sparkles backdrop */}
            <div className="absolute right-4 top-1/2 -translate-y-1/2 text-indigo-500/10 dark:text-indigo-400/10 pointer-events-none select-none">
      ...
      {(subjectDocuments.length > 2 || activeSubjectId === 'COMPSCI') && (
        <div className="space-y-4 pt-1">
          <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
            ...
            <button
              onClick={() => navigate('/dashboard/documents')}
              className="text-xs font-extrabold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors focus:outline-none cursor-pointer"
            >
              View All
            </button>
          </div>
      ...
      <div className="pt-2">
        <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-xs flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden min-h-[150px]">
      ...
      {showFilters && (
        <div ref={filterContainerRef} className="flex flex-col gap-4 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-xs md:flex-row md:items-center md:justify-between animate-fade-in">
          {/* Search field */}
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950 pl-10 pr-4 py-2.5 text-sm text-slate-700 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-[#2563eb]/50 focus:bg-white dark:focus:bg-slate-950 focus:outline-none focus:ring-2 focus:ring-[#2563eb]/10 transition-all"
            />
            {searchQuery && (
              <button
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-955 p-1">
              <button
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          <h2 className="text-[11px] font-extrabold tracking-wider text-slate-400 dark:text-slate-500 uppercase">
        ...
        {filteredDocuments.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 py-16 px-4 text-center shadow-xs">
            <div className="flex h-14 w-14 items-center justify-center rounded-full bg-blue-50 dark:bg-blue-955/50 text-[#2563eb] dark:text-blue-400">
            ...
            {filteredDocuments.map((doc) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between" onClick={(e) => e.stopPropagation()}>
                      ...
                      <div className="relative">
                        <button
                          ...
                          className="menu-trigger-btn rounded-lg p-1.5 text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-600 dark:hover:text-slate-250 focus:outline-none transition-colors"
                        >
                    ...
                    <div className="mt-4 flex-1">
                      <h3 className="text-base font-extrabold text-slate-800 dark:text-slate-100 group-hover:text-[#2563eb] dark:group-hover:text-blue-400 transition-colors leading-tight truncate">
                        {doc.title || doc.fileName}
                      </h3>
                      <p className="mt-1 text-[13px] text-slate-400 dark:text-slate-500 line-clamp-2 leading-normal font-medium">
                        Comprehensive guide to Creational, Structural, and Behavioral patterns in Java.
                      </p>
                    </div>

                    {/* Footer with avatar list */}
                    <div className="mt-4 flex items-center justify-between border-t border-slate-100 dark:border-slate-800/60 pt-3" onClick={(e) => e.stopPropagation()}>
                      <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">Updated 2h ago</span>
                      <div className="flex -space-x-1.5 overflow-hidden">
                        <div className="inline-flex h-6.5 w-6.5 items-center justify-center rounded-full bg-gradient-to-br from-teal-400 to-emerald-500 text-white text-[9px] font-black border-2 border-white dark:border-slate-900 shadow-xs" title="John Doe">JD</div>
                        <div className="inline-flex h-6.5 w-6.5 items-center justify-center rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 text-white text-[9px] font-black border-2 border-white dark:border-slate-900 shadow-xs" title="Alice Lam">AL</div>
                      </div>
              ...
              if (doc.id === 'doc-agile') {
                return (
                  ...
                  >
                    ...
                    <div className="mt-4 flex items-center justify-between border-t border-slate-100 dark:border-slate-800/60 pt-3" onClick={(e) => e.stopPropagation()}>
                      <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">Updated Yesterday</span>
                      <span className="rounded bg-[#F5F3FF] dark:bg-purple-955/40 border border-[#DDD6FE]/40 dark:border-purple-900/30 px-2 py-0.5 text-[9px] font-bold text-[#7C3AED] dark:text-purple-400 tracking-wider uppercase">
                        ESSENTIAL
                      </span>
                    </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-4 flex-1">
                    <h3 className="line-clamp-1 text-[15px] font-bold text-slate-800 dark:text-slate-100 group-hover:text-[#2563eb] dark:group-hover:text-blue-400 transition-colors" title={doc.title || doc.fileName}>
                      {doc.title || doc.fileName}
                    </h3>
                    <p className="mt-1 text-xs text-slate-400 dark:text-slate-550 font-medium">
                      {doc.fileName}
                    </p>
                    <p className="mt-1 text-xs text-slate-400 dark:text-slate-500 flex items-center gap-1 font-medium">
                      {doc.uploadedAt}
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-slate-100 dark:border-slate-800/60 pt-3">
                    <span className="rounded-md bg-blue-50/70 dark:bg-blue-955/40 border border-blue-100/50 dark:border-blue-900/30 px-2 py-0.5 text-[10px] font-bold tracking-wider text-[#2563eb] dark:text-blue-400">
                      {doc.subject}
                    </span>
                    {renderStatusBadge(doc.status)}
                  </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs">
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-850 text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {filteredDocuments.map((doc) => (
                    <tr
                      key={doc.id}
                      className="group hover:bg-slate-50/30 dark:hover:bg-slate-850/30 transition-colors cursor-pointer"
                      onClick={() => handleOpenDocument(doc.id)}
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          {renderFileIcon(doc.type)}
                          <div>
                            <h4 className="text-[14px] font-bold text-slate-800 dark:text-slate-200 group-hover:text-[#2563eb] dark:group-hover:text-blue-400 transition-colors">
                              {doc.title || doc.fileName}
                            </h4>
                      ...
                      <td className="px-6 py-4">
                        <span className="rounded-md bg-blue-50/70 dark:bg-blue-955/40 border border-blue-100/50 dark:border-blue-900/30 px-2.5 py-0.5 text-[10px] font-bold tracking-wider text-[#2563eb] dark:text-blue-400">
                          {doc.subject}
                        </span>
                      </td>
