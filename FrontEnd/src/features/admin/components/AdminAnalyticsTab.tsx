...
// Custom tooltips
const TrafficTooltip = ({ active, payload }: any) => {
    ...
    return (
      <div className="bg-slate-900/90 dark:bg-slate-950/90 backdrop-blur-md text-white px-3.5 py-2.5 rounded-2xl text-xs font-semibold shadow-xl border border-white/10 flex flex-col gap-1 select-none animate-fade-in">
        ...
        <div className="flex items-center gap-2">
          ...
          <span>Page Views: <span className="font-extrabold">{payload[0].value.toLocaleString()}</span></span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-emerald-500" />
          <span>AI Queries: <span className="font-extrabold">{payload[1].value.toLocaleString()}</span></span>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="bg-slate-900/90 dark:bg-slate-950/90 backdrop-blur-md text-white px-3.5 py-2.5 rounded-2xl text-xs font-semibold shadow-xl border border-white/10 flex items-center gap-2 select-none animate-fade-in">
        ...
        <div>
          ...
          <span className="text-[10px] text-slate-450 ml-1.5 font-medium">interactions</span>
        </div>
      </div>
    )
  }
...
export function AdminAnalyticsTab() {
  ...
  const moduleUsageData = useMemo(() => {
    return [
      { name: language === 'vi' ? 'Trò chuyện AI' : 'AI Chatbot', value: 85240 },
      { name: language === 'vi' ? 'Lưu trữ tệp' : 'File Storage', value: 64205 },
      { name: language === 'vi' ? 'Kế hoạch học' : 'Study Plans', value: 38450 },
      { name: language === 'vi' ? 'Quản lý tài liệu' : 'Documents', value: 29400 }
    ]
  }, [language])
