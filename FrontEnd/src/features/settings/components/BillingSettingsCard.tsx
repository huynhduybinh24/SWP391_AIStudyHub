...
import { CreditCard, Calendar, RefreshCw, Sparkles, Ban, CheckCircle2 } from 'lucide-react'
...
export function BillingSettingsCard() {
  ...
  useEffect(() => {
    if (user?.plan === 'pro') {
      ...
      if (storedExpiry) {
        ...
        const defaultExpiry = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toLocaleDateString(
  ...
  const handleSimulateExpiry = async () => {
    ...
    if (isAutoRenew) {
      ...
      // Send System Notification
      const title = language === 'vi' ? 'Gia hạn gói Pro thất bại (Hết hạn)' : 'Pro Subscription Expired'
      const msg = language === 'vi'
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                  {language === 'vi' ? 'GÓI MIỄN PHÍ' : 'FREE TIER MEMBER'}
                </span>
              </div>
          ...
          <div className="rounded-2xl border border-blue-500/20 bg-blue-500/5 dark:bg-blue-500/10 p-5 flex flex-col md:flex-row justify-between items-center gap-5">
            ...
            <Link
              ...
              className="w-full md:w-auto text-center px-6 py-3.5 rounded-xl text-xs font-black bg-gradient-to-r from-blue-600 to-indigo-650 hover:from-blue-750 hover:to-indigo-700 text-white shadow-md shadow-blue-500/10 hover:shadow-lg transition-all no-underline inline-block active:scale-[0.98]"
            >
